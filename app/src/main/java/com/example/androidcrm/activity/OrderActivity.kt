                ...
                result.data?.let { data ->
                    ...
                    if (title != null && customerId != -1) {
                        ...
                        if (orderId == -1) {
                            ...
                            Snackbar.make(recyclerView, "Order saved", Snackbar.LENGTH_SHORT).show()
                        } else {
                            order.id = orderId
                            orderViewModel.updateOrder(order)
                            Snackbar.make(recyclerView, "Order updated", Snackbar.LENGTH_SHORT).show()
                        }
                    } else {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.delete_all -> {
                ...
                Snackbar.make(recyclerView, "All orders deleted", Snackbar.LENGTH_SHORT).show()
                true
            }
